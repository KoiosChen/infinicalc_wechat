...
def fill_relationship_rebate(relationships, item_rebate_policy):
...
def checkout_rebates_ratio(customer, shop_order_id, pay_type=None):
    try:
...
def calc(order_id, customer, pay_type=None):
    def newPersonalRebates(rebate_relation_id, relation_):
        if 'rebate' in detail.keys() or 'score' in detail.keys():
            ...
            if pay_type is None:
                ...
                data['wechat_pay_id'] = order_id
            new_personal_rebate = new_data_obj('PersonalRebates', **data)
    ...
    try:
        rebate_ratio = checkout_rebates_ratio(customer, order_id, pay_type=pay_type)
